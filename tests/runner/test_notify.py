...
def test_entry_and_exit_formatting(monkeypatch):
    monkeypatch.setenv("TONY_NOTIFY", "telegram")
    monkeypatch.setenv(
        "TONY_NOTIFY_POLICY", "off"
    )  # asserts reprice FORMAT; bypass the dedup gate
    ...
    nf.notify_entry(
        "NVDA", 66, 100.0, 94.0, 115.0
    )  # default risk_pct=1.0 -> "1% of the account"
    ...
    assert (
        "I bought NVDA" in msgs[0]
        and "$100.00" in msgs[0]
        and "1% of the account" in msgs[0]
    )
    assert "100%" not in msgs[0]  # regression guard for the label bug
    assert (
        "I sold NVDA for a +$924 win" in msgs[1]
        and "price target" in msgs[1]
        and "2.3×" in msgs[1]
    )
    assert "I sold FCX for a $310 loss" in msgs[2] and "safety stop" in msgs[2]
    assert (
        "I adjusted FCX" in msgs[3]
        and "stop $61.88" in msgs[3]
        and "target $74.62" in msgs[3]
    )
...
def test_notify_chat_override_and_markup(monkeypatch):
    ...
    assert (
        sent["reply_markup"]["inline_keyboard"][0][0]["callback_data"] == "cmd:status"
    )
...
def test_closed_positions_detects_exits():
    prior = [
        {"symbol": "AAA", "qty": 10, "avg_entry_price": 5.0},
