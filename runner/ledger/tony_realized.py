...
def _load() -> list:
    return load_list(REALIZED_FILE)


def record_realized(
    symbol, qty, entry, exit_price, target=None, stop=None
) -> dict | None:
    ...
    row = {
        ...
        "pct": pct,
        "reason": infer_reason(ex, target, stop),
        "date": _trading_day(),
    }
    rows = _load()
    rows.append(row)
    try:
        atomic_write_json(REALIZED_FILE, rows, indent=2)
...
def _agg(rows: list) -> dict:
    ...
    for r in rows:
        by_reason[r.get("reason", "unknown")] = (
            by_reason.get(r.get("reason", "unknown"), 0) + 1
        )
    return {
        "count": len(closed),
        ...
        "realized_pl": round(
            sum(_pl(r) for r in rows), 2
        ),  # total incl. trims (real money)
        "closed_pl": round(
            sum(_pl(r) for r in closed), 2
        ),  # P/L from actual closes only
        "trims": len(trims),
...
def summary() -> dict:
    ...
    today = (
        _trading_day()
    )  # ET — a 9 PM ET wrap must still see the day's exits (UTC has rolled)
    return {
        "today": _agg([r for r in rows if r.get("date") == today]),
        "all_time": _agg(rows),
    }
...
def records(newest_first: bool = True) -> list:
    ...
    rows = sorted(
        _load(), key=lambda r: (str(r.get("date", "")), str(r.get("symbol", "")))
    )
    ...
    for f in sorted(fills, key=lambda x: str(x.get("time", ""))):
        ...
        if side == "buy":
            ...
            reason = (
                "stop"
                if ot == "stop"
                else ("target" if ot in ("limit", "take_profit") else "close")
            )
            ...
            rows.append(
                {
                    ...
                    "exit": round(price, 4),
                    "realized_pl": round((price - avg_entry) * matched, 2),
                    "pct": round((price - avg_entry) / avg_entry * 100, 2)
                    if avg_entry
                    else 0.0,
                    "reason": reason,
                    "date": f.get("date"),
                    "exit_order_id": f.get("order_id"),
                }
            )
    return rows
...
def rebuild_from_fills(fills: list) -> dict:
    ...
    kept = [
        r
        for r in _load()
        if r.get("exit_order_id") and r.get("exit_order_id") not in rec_ids
    ]
    ...
    try:
        atomic_write_json(REALIZED_FILE, merged, indent=2)
    except OSError as exc:
        _log.warning("realized rebuild write failed: %s", exc)
    return {
        "records": len(merged),
        "added": len(merged) - prior_count,
        "realized_pl": round(
            sum(float(r.get("realized_pl", 0) or 0) for r in merged), 2
        ),
    }
