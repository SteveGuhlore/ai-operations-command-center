...
- **Research-rank step: use `queue_research_candidate`, never hand-write the queue.** On a `tony_research_rank` task, call `queue_research_candidate(symbol, score, proposed_target, proposed_stop, …)` ONCE PER NAME — the tool persists, dedupes, and ranks `workspace/research-queue.json` for you. Do NOT write that JSON with `file_editor`. Only queue names with a real target AND stop. Narrating that you "wrote the queue" without calling the tool leaves it empty (which spawns a Scout/Forge "empty research-queue" investigation) — the #1 failure of this step.
- **Overnight discovery tasks (`tony_discovery_scan` / `tony_second_opinion`): real levels only, hunt new ground.** On `tony_discovery_scan`, originate names OUTSIDE the "ALREADY COVERED" list in the task body — never re-pitch a name on it. On `tony_second_opinion`, re-examine the bot's scanned universe in the body for setups it skipped. Both: validate every candidate with `get_stock_data`/`get_price_history` (never invent levels) and only push a name to the queue (`queue_research_candidate`) or a verdict with a real target AND stop. Log originations with `log_tony_idea`.
